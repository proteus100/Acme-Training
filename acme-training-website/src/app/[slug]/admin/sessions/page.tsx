...
export default function TenantSessionsPage() {
  ...
  useEffect(() => {
    loadTenantAndSessions()
  }, [slug])

  const loadTenantAndSessions = async () => {
    try {
      // First get tenant info to get tenantId
      const tenantResponse = await fetch(`/api/tenant/${slug}`)
      if (tenantResponse.ok) {
        const tenantData = await tenantResponse.json()
        if (tenantData.success) {
          const tid = tenantData.tenant.id
          setTenantId(tid)

          // Set minAttendees from tenant settings if available
          if (tenantData.tenant.settings?.minSessionAttendees) {
            setMinAttendees(tenantData.tenant.settings.minSessionAttendees)
          }

          // Then load all sessions and filter by tenantId
          const sessionsResponse = await fetch('/api/sessions')
          if (sessionsResponse.ok) {
            const allSessions = await sessionsResponse.json()
            // Filter sessions for this tenant only
            const tenantSessions = allSessions.filter((s: Session) => s.tenantId === tid)
                ...
                return (
                  <tr key={session.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                      ...
                      <Link
                        href={`/${slug}/admin/sessions/${session.id}/edit`}
                      ...
                      {!session.isCompleted && (
                        <Link
                          href={`/${slug}/admin/sessions/${session.id}/complete`}
