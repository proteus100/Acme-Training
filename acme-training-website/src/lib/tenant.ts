...
export function extractTenantFromRequest(hostname: string, pathname: string): {
  ...
  // Determine if this is an admin/API route
  const isAdminRoute = pathname.startsWith('/admin') ||
    pathname.startsWith('/onboarding') ||
    pathname.includes('/admin/') ||  // Include tenant admin routes like /{slug}/admin
    (pathname.startsWith('/api') && !pathname.startsWith('/api/tenant/'))

  // Extract tenant slug from hostname regardless of route type
